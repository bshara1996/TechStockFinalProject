...
exports.getAllProducts = async (req, res) => {
  try {
    ...
    if (manufacturer && manufacturer.length > 0) {
      const manufacturerIds = Array.isArray(manufacturer)
        ? manufacturer
        : [manufacturer];
    ...
    const joinClause =
      "LEFT JOIN categories c ON p.category_id = c.category_id LEFT JOIN suppliers s ON p.supplier_id = s.supplier_id";
    const whereClause =
    ...
    // Get paginated products (newest products first by product_id)
...
exports.getPriceStats = async (req, res) => {
  try {
    ...
    console.log("getPriceStats result:", {
      minPrice,
      maxPrice,
      category,
      manufacturer,
...
exports.getProductsByIds = async (req, res) => {
  ...
  try {
    ...
    if (idArray.length === 0) {
      return res
        .status(400)
        .json({ message: "Valid product IDs are required." });
    }
...
exports.createProduct = async (req, res) => {
  const { name, description, price, stock, supplier_id, category_id } =
    req.body;
...
exports.getAllProductsAdmin = async (req, res) => {
  try {
    ...
    console.log("Sort params received:", {
      sortField,
      normalizedField,
      orderByColumn,
      direction,
    ...
    if (orderByColumn === "p.is_active") {
      ...
      console.log("Status sorting applied:", {
        direction,
        orderByClause,
        sortField,
        normalizedField,
