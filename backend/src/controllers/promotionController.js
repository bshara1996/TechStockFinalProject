  ...
  try {
    ...
    if (search) {
      whereConditions.push(
        "(name LIKE ? OR description LIKE ? OR code LIKE ?)"
      );
...
exports.createPromotion = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        message:
          "Promotion must be applied to at least one specific product or category. General promotions are not allowed.",
      });
    }
    ...
    if (existingPromotion.length > 0) {
      return res.status(400).json({
        message:
          "A promotion with this name already exists. Please choose a different name.",
      });
    }
