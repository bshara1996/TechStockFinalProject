...
const loadUserCart = (req) => {
  ...
  if (!req.session.userCarts[userId]) {
    ...
    console.log(
      `Cart: Loaded existing cart for user ${userId} with ${req.session.userCarts[userId].items.length} items`
    );
  }
...
const saveUserCart = (req) => {
  ...
  if (req.session.cart) {
    ...
    console.log(
      `Cart: Saved cart for user ${userId} with ${req.session.cart.items.length} items`
    );
  }
};
...
const addToSessionCart = async (req, res) => {
  try {
    ...
    if (existingItemIndex >= 0) {
      ...
      req.session.cart.items[existingItemIndex].category_id =
  ...
  try {
    ...
    if (quantity === 0) {
      ...
      if (products[0].stock < quantity) {
        return res
          .status(400)
          .json({ message: `Only ${products[0].stock} units available` });
      }
...
const validateSessionCart = async (req, res) => {
  try {
    ...
    const currentProductsMap = new Map(
      currentProducts.map((p) => [p.product_id, p])
    );
...
const applyPromotionToSessionCart = async (req, res) => {
  try {
    ...
    try {
      ...
      const totalCartAmount = req.session.cart.items.reduce(
      ...
      if (applicableItems.length === 0) {
        return res
          .status(400)
          .json({ message: "No items are eligible for this promotion." });
      }

      // For fixed value promotions, check if product price is lower than the fixed promotion value
      if (promotion.type === "fixed") {
        const promotionValue = parseFloat(promotion.value);
        const invalidItems = applicableItems.filter(
          (item) => parseFloat(item.price) < promotionValue
        );
        if (invalidItems.length > 0) {
          const itemNames = invalidItems.map((item) => item.name).join(", ");
          console.log(
            `Debug - Item prices: ${invalidItems
              .map((i) => i.price)
              .join(", ")}, Promotion value: ${promotionValue}`
          );
          return res.status(400).json({
            message: `Promotion failed: item prices are below the promotion value. [${itemNames}]`,
          });
        }
      }
      ...
      if (
        ...
        totalApplicableQuantity < promotion.min_quantity
      ) {
        return res.status(400).json({
      ...
      if (
        ...
        totalApplicableQuantity > promotion.max_quantity
      ) {
        return res.status(400).json({
      ...
      if (promotion.type === "percentage") {
        ...
        discount = parseFloat(promotion.value) * totalApplicableQuantity;
      }
