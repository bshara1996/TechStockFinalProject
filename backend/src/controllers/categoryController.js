...
exports.getPublicCategories = async (req, res) => {
  try {
    ...
    if (includeInactive === "true") {
      ...
      query =
        "SELECT *, isActive FROM categories WHERE isActive = TRUE ORDER BY name";
    }
...
exports.getCategoriesByIds = async (req, res) => {
  ...
  try {
    ...
    if (idArray.length === 0) {
      return res
        .status(400)
        .json({ message: "Valid category IDs are required." });
    }
...
exports.createCategory = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to add category due to a server error." });
  }
};
...
exports.updateCategory = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to update category due to a server error." });
  }
};
...
exports.deleteCategory = async (req, res) => {
  ...
  try {
    ...
    const [result] = await db.query(
      "UPDATE categories SET isActive = FALSE WHERE category_id = ?",
      [id]
    );
    if (result.affectedRows === 0) {
      return res.status(404).json({ message: "Category not found." });
    ...
    res.status(500).json({
      message: "Failed to deactivate category due to a server error.",
    });
  }
};
...
exports.restoreCategory = async (req, res) => {
  ...
  try {
    const [result] = await db.query(
      "UPDATE categories SET isActive = TRUE WHERE category_id = ?",
      [id]
    );
    if (result.affectedRows === 0) {
      return res.status(404).json({ message: "Category not found." });
    ...
    res
      .status(500)
      .json({ message: "Failed to restore category due to a server error." });
  }
};
...
exports.toggleCategoryStatus = async (req, res) => {
  ...
  try {
    ...
    const [current] = await db.query(
      "SELECT isActive FROM categories WHERE category_id = ?",
      [id]
    );
    if (current.length === 0) {
      return res.status(404).json({ message: "Category not found." });
    ...
    const [result] = await db.query(
      "UPDATE categories SET isActive = ? WHERE category_id = ?",
      [newStatus, id]
    );
    ...
    res.status(500).json({
      message: "Failed to toggle category status due to a server error.",
    });
  }
};
