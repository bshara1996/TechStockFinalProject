  ...
  try {
    ...
    if (search) {
      whereConditions.push(
        "(u.name LIKE ? OR u.email LIKE ? OR o.order_id LIKE ?)"
      );
    ...
    const normalizedField = String(sortField || "")
      .toLowerCase()
      .trim();
...
exports.getCustomers = async (req, res) => {
  try {
    const {
      ...
      sortField = "user_id",
      sortDirection = "desc",
    } = req.query; // Search, pagination, filter, sorting
  ...
  try {
    ...
    if (ordersCheck[0].non_delivered_count > 0) {
      return res.status(400).json({
        message:
          "Cannot delete customer with non-delivered orders. All orders must be in Delivered status first.",
      });
    }
