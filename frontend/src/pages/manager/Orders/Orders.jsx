...
const Orders = () => {
      ...
      requestAnimationFrame(() => {
        if (searchInputRef.current) {
          const cursorPosition =
            searchInputRef.current.selectionStart || searchTerm.length;
  ...
  // Fetch statistics, optionally filtered by search (email/name/order id) and userId
  const fetchStatistics = useCallback(async (opts = {}) => {
    try {
      const { search = "", userId = "" } = opts;
  ...
  useEffect(() => {
    ...
    fetchStatistics({ search: urlSearch, userId: urlUserId });
      ...
      if (isUserAdmin && !loadingSettings && searchTerm.trim()) {
        ...
        const performSearch = async () => {
          try {
            ...
            // Update statistics to reflect search filter and current user filter
            await fetchStatistics({
              search: searchTerm.trim(),
              userId: userFilter || searchParams.get("userId") || "",
            });
        ...
        const loadCurrentPage = async () => {
          try {
            ...
            // Update statistics to reflect cleared search and current user filter
            await fetchStatistics({ search: "", userId: currentUserId });
  ...
  const handleUserFilterChange = useCallback(
    async (newUserId) => {
      ...
      // Immediately refresh statistics to reflect the selected user/email
      await fetchStatistics({ search: searchTerm.trim(), userId: newUserId });
      // Orders will be refreshed by the main URL effect (or pagination handlers)
    },
    [searchParams, setSearchParams, fetchStatistics, searchTerm]
  );
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        const response = await fetch("/api/admin/customers?limit=1000&page=1", {
          headers,
        });
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    ...
    try {
      ...
      const urlPage =
        parseInt(searchParams.get("page")) || pagination.currentPage;
      const urlSearch = searchParams.get("search") || "";
      const urlStatus = searchParams.get("status") || "all";
      const urlUserId = searchParams.get("userId") || "all";
      const urlSortField = searchParams.get("sortField");
  ...
  return (
    <div className="orders-page">
      <div className="orders-header">
        <h1>
          <IoDocumentsSharp color="#363636b7" /> Manage Orders
        </h1>
        ...
        <table className="orders-table">
          <thead>
            <tr>
              ...
              <th
                className={`sortable ${
                  sortField === "customer_name" ? "active" : ""
                }`}
                ...
                return (
                  <tr key={order.order_id}>
                    ...
                    <td className="order-actions">
                      <button onClick={() => handleOrderClick(order.order_id)}>
                        View Details
                      </button>
                    </td>
                  </tr>
      ...
      {showModal && selectedOrder && (
        <div className="modal-overlay" onClick={closeModal}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <p>
              <strong>Date:</strong> {formatDateTime(selectedOrder.order_date)}
            </p>
            <p>
              <strong>Total:</strong>{" "}
              {formatPriceConverted(selectedOrder.total_price, currency)}
            </p>
            {selectedOrder.promotion_code &&
              (() => {
                // Prefer stored discount_amount (DB) which is tax-inclusive (gross)
                const storedDiscount =
                  typeof selectedOrder.discount_amount !== "undefined" &&
                    ...
                    <p>
                      ...
                      {storedDiscount > 0 && (
                        <span className="promo-discount">
                          ...
                          {formatPriceConverted(storedDiscount, currency)}
                        </span>
                      )}
                    </p>
                  );
                ...
                return (
                  <li key={idx} className="order-product-item">
                    ...
                    <div className="product-meta">
                      Checkout Unit Price:{" "}
                      {formatPriceConverted(unitPrice, currency)} (x
                      {item.quantity})
                    </div>
                    <div className="product-meta">
                      Total: {formatPriceConverted(totalPrice, currency)}
                    </div>
                  </li>
                );
              })}
