...
import { FaSort, FaSortDown, FaSortUp, FaEdit, FaTrash } from "react-icons/fa";
import { MdLocalShipping, MdOutlineAutorenew } from "react-icons/md";
...
const Suppliers = () => {
  ...
  const validatePhone = (phone) => {
    ...
    if (
      phoneDigits.length ===
      phoneDigits.split("").filter((d) => d === phoneDigits[0]).length
    ) {
  ...
  const formatPhoneNumber = (value) => {
    ...
    if (phoneNumber.length <= 3) {
      ...
      )} (${phoneNumber.slice(-10, -7)}) ${phoneNumber.slice(
        -7,
        -4
      )}-${phoneNumber.slice(-4)}`;
    }
  ...
  useEffect(() => {
    ...
    // Prepare query parameters and handle loading, errors, and auth token before fetching suppliers
  ...
  const handleUpdateSupplier = async (e) => {
    ...
    try {
      ...
      setSuppliers((prev) =>
        prev.map((s) =>
          s.supplier_id === updated.supplier_id ? { ...s, ...updated } : s
        )
      );
    ...
    showConfirm(
      ...
      async () => {
        try {
          ...
          showError(
            err.response?.data?.message || "Failed to restore supplier."
          );
        }
      }
    );
  ...
  return (
    <div className="suppliers-container">
      <h1 className="suppliers-title">
        <MdLocalShipping color="#363636b7" /> Manage Suppliers
      </h1>
      <p className="suppliers-subtitle">
        Add, edit, or delete product suppliers.
      </p>
      ...
      <div className="table-container">
        <table className="suppliers-table">
          ...
          <tbody>
            {suppliers.length === 0 ? (
              <tr>
                <td colSpan="7" className="no-suppliers">
                  No suppliers found.
                </td>
              </tr>
            ) : (
              suppliers.map((supplier) => (
                <tr
                  ...
                  className={supplier.isActive === 0 ? "inactive-row" : ""}
                >
                  ...
                  <td className="actions-cell">
                    {supplier.isActive === 1 ? (
                      ...
                      <>
                        ...
                        <button
                          onClick={() =>
                            handleRestoreSupplier(supplier.supplier_id)
                          }
            ...
            <form onSubmit={handleAddSupplier} className="modal-form">
              ...
              <div className="form-group">
                ...
                {phoneValidationError && (
                  <span className="field-error">{phoneValidationError}</span>
                )}
              </div>
            ...
            <form onSubmit={handleUpdateSupplier} className="edit-modal-form">
              ...
              <div className="edit-form-group">
                ...
                <input
                  ...
                  className={`edit-form-input ${
                    editPhoneValidationError ? "error" : ""
                  }`}
                />
              ...
              <div className="edit-modal-buttons">
                ...
                <button type="submit" className="edit-modal-button primary">
                  Update Supplier
                </button>
              </div>
            </form>
