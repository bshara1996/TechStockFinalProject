...
const Customers = () => {
        ...
        const performSearch = async () => {
          try {
            ...
            const response = await fetch(
              `/api/admin/customers?${fetchParams}`,
              {
                headers: {
                  Authorization: `Bearer ${token}`,
                },
    ...
    const loadCustomers = async () => {
      try {
        ...
        if (urlSortField && urlSortDirection) {
          ...
          params.append(
            "sortDirection",
            urlSortDirection === "desc" ? "desc" : "asc"
          );
        }
  ...
  const fetchCustomerOrders = async (userId) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `/api/admin/orders?userId=${userId}&limit=10000`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const fetchOrderItems = async (orderId) => {
    try {
      ...
      const data = await response.json();
      // Store both products and promotion info
      setOrderItems((prev) => ({
        ...prev,
        [orderId]: {
      ...
      async () => {
        try {
          ...
          if (!response.ok) {
            ...
            throw new Error(
              errorData.message || "Failed to deactivate customer"
            );
          }
    ...
    showConfirm(
      ...
      async () => {
        try {
          const token = localStorage.getItem("token");
          const response = await fetch(
            `/api/admin/customers/${customer.user_id}/restore`,
            {
              method: "PATCH",
              headers: {
                Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="customers-container">
      <h1 className="customers-title">
        <FaUserGroup color="#363636b7" /> Manage Customers
      </h1>
        ...
        <div className="status-filter-section">
          <div className="status-filter-buttons">
            <button
              ...
              className={`status-filter-btn all ${
            ...
            <button
              ...
              className={`status-filter-btn active ${
                statusFilter === "active" ? "active-state" : ""
              }`}
            >
            ...
            <button
              ...
              className={`status-filter-btn inactive ${
                statusFilter === "inactive" ? "active-state" : ""
              }`}
            >
            ...
            {customers.map((customer) => (
              <tr
                ...
                className={`table-body-row ${
                  customer.isActive === 0 ? "inactive-row" : ""
                }`}
              >
                ...
                <td className="table-body-cell">
                  {formatPriceConverted(customer.total_spent || 0, currency)}
                </td>
                <td className="table-body-cell">
                  <span
          ...
          <div className="modal-content">
            <div className="modal-header">
              <h2 className="modal-title">
                Orders for {selectedCustomer.username}
              </h2>
              <button
                    ...
                    {expandedOrderId === order.order_id && (
                      <div className="order-items-dropdown">
                        {orderItems[order.order_id] ? (
                          (() => {
                            const orderData = orderItems[order.order_id];
                            const products = Array.isArray(orderData)
                              ? orderData
                              : orderData.products || [];
                            // Get promotion info from order or stored data
                            const promotion =
                              orderData.promotion &&
                              orderData.promotion.promotion_code
                                ? orderData.promotion
                                ...
                                ? {
                                    ...
                                    applicable_products:
                                      order.applicable_products,
                                    applicable_categories:
                                      order.applicable_categories,
                            ...
                            return products.length > 0 ? (
                              <>
                                {promotion?.promotion_code &&
                                  (() => {
                                    // Prefer stored order.discount_amount (tax-inclusive) if present
                                    const storedDiscount =
                                      typeof order.discount_amount !==
                                        "undefined" &&
                                    ...
                                    return (
                                      <div className="promotion-code-container">
                                        ...
                                        <span className="promotion-badge promotion-badge-margin">
                                          {promotion.promotion_code}
                                        </span>
                                        {(storedDiscount !== null
                                          ? storedDiscount
                                          : 0) > 0 && (
                                          <span className="discount-amount-text">
                                ...
                                <div className="items-list">
                                  {products.map((item, index) => {
                                    // Calculate discount (simplified - we need product_id and category_id)
                                    ...
                                    return (
                                      <div
                                        key={index}
                                        className="order-item-detail order-item-detail-wrapper"
                                      >
