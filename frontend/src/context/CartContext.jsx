...
export const CartProvider = ({ children }) => {
  ...
  const validateCart = async (showNotifications = true, force = false) => {
    ...
    const now = Date.now();
    if (!force && now - lastValidationTime.current < 60000) {
    ...
    try {
      ...
      // Update promotion state if provided
        ...
        if (showNotifications) {
          ...
          if (removedItems.length > 0) {
            const removedNames = removedItems
              ...
              .join(", ");
            showError(
              `Removed from cart (no longer available): ${removedNames}`
            );
          }
        }
      }

      // Show promotion message if promotion was removed
      if (promotionMessage && showNotifications) {
        showInfo(promotionMessage);
      }
    } catch (error) {
  ...
  const addToCart = async (product, quantity = 1) => {
    if (!product.stock || product.stock < 0) {
      showError(
        "This product has invalid inventory data and cannot be added to cart."
      );
      return;
    }
    ...
    if (quantity > product.stock) {
      showError(
        `Only ${product.stock} unit${
          product.stock === 1 ? "" : "s"
        } available. Cannot add ${quantity} to cart.`
      );
      return;
    }
    ...
    try {
      ...
      // Update promotion state if provided
      if (response.data.appliedPromotion !== undefined) {
        setAppliedPromotion(response.data.appliedPromotion);
      }
      if (response.data.discountAmount !== undefined) {
        setDiscountAmount(response.data.discountAmount || 0);
      }

      // Show promotion message if promotion was removed
      if (response.data.promotionMessage) {
        showInfo(response.data.promotionMessage);
      }
      ...
      if (user_id) {
        ...
        localStorage.setItem(
          ...
          JSON.stringify({
            ...
            appliedPromotion:
              response.data.appliedPromotion !== undefined
                ? response.data.appliedPromotion
                : appliedPromotion,
            discountAmount:
  ...
  const applyPromotion = async (promotionCode) => {
    try {
      ...
      const { appliedPromotion: newPromotion, discountAmount: newDiscount } =
        response.data;
      ...
      if (newPromotion) {
        if (discountCapped) {
          ...
          const formattedSubtotal = formatPriceConverted(
            currentSubtotal,
            currency
          );
          showInfo(
            `Promotion "${newPromotion.name}" applied, but discount was capped at subtotal amount (${formattedSubtotal}).`
          );
        } else {
