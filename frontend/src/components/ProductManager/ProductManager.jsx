...
function ProductManager() {
  ...
  const prevStatsParamsRef = useRef("");
  ...
  const handleSort = (field) => {
    ...
    const currentUiField =
      urlSortField === "category_name"
        ? "category_id"
        : urlSortField === "supplier_name"
        ? "supplier_id"
        : urlSortField === "status"
        ? "is_active"
  ...
  const fetchGlobalStats = useCallback(
    async ({ searchOverride, categoryOverride, supplierOverride } = {}) => {
      ...
      try {
        ...
        const urlSupplierFilter =
          supplierOverride !== undefined
            ? supplierOverride
        ...
        const response = await fetch(
          `/api/products/admin/stats?${params.toString()}`,
          { headers }
        );
        if (!response.ok) {
  ...
  const getStockLevel = (stock) => {
    ...
    if (stock <= 10)
      return { level: "low", class: "stock-low-stock", text: "LOW STOCK" };
    if (stock <= 20)
      return {
        level: "medium",
        class: "stock-medium-stock",
        text: "MEDIUM STOCK",
      };
  ...
  const fetchAllSuppliers = useCallback(async (headers) => {
    try {
      const firstPageRes = await fetch("/api/suppliers?limit=10000&page=1", {
        headers,
      });
      ...
      let allSuppliers =
        firstPageData.suppliers ||
        (Array.isArray(firstPageData) ? firstPageData : []);

      // If there are more pages, fetch them all
      if (firstPageData.pagination && firstPageData.pagination.totalPages > 1) {
        const additionalPages = await Promise.all(
          Array.from(
            { length: firstPageData.pagination.totalPages - 1 },
            (_, i) =>
  ...
  const fetchAllCategories = useCallback(async (headers) => {
    try {
      const firstPageRes = await fetch("/api/categories?limit=10000&page=1", {
        headers,
      });
  ...
  const fetchProductData = useCallback(
    async (pageOverride = null, searchOverride = null) => {
      ...
      try {
        ...
        const productsRes = await fetch(`/api/products/admin/all?${params}`, {
          headers,
        });
        if (!productsRes.ok) {
  ...
  ); // Read all from URL, no need for filter state
  ...
  // Refresh stats when filters or search change (but not when only status changes)
  // Track a compact key of the stats-relevant URL params (search, category, supplier)
  useEffect(() => {
    const currentSearch = searchParams.get("search") || "";
    const urlCategoryFilter = searchParams.get("category") || "all";
    const urlSupplierFilter = searchParams.get("supplier") || "all";

    const relevantKey = `${currentSearch}||${urlCategoryFilter}||${urlSupplierFilter}`;

    if (prevStatsParamsRef.current !== relevantKey) {
      prevStatsParamsRef.current = relevantKey;
      fetchGlobalStats();
    }
    // If only status changed, relevantKey will be identical and we skip fetching stats
  ...
  useEffect(() => {
    ...
    const shouldUpdate =
      ...
      (urlSortField &&
        (!sortConfig.field ||
          mapToApiSortField(sortConfig.field) !== urlSortField ||
          sortConfig.direction !== (urlSortDirection || "asc"))) ||
    ...
    if (urlSortField) {
      ...
      setSortConfig({
        field: uiField,
        direction: urlSortDirection === "desc" ? "desc" : "asc",
      });
    } else {
    ...
    console.log(
      "About to call fetchProductData with page:",
      currentPage,
      "search:",
      currentSearch
    );
      ...
      if (isUserAdmin && !loadingSettings && searchTerm.trim()) {
        ...
        const performSearch = async () => {
          try {
            ...
            const response = await fetch(
              `/api/products/admin/all?${fetchParams}`,
              { headers }
            );
            ...
            // Update stats to reflect the search input (exclude status)
            try {
              await fetchGlobalStats({ searchOverride: searchTerm.trim() });
            } catch (err) {
              console.error("Error updating stats after search:", err);
            }
        ...
        const loadCurrentPage = async () => {
          try {
            ...
            // When clearing search, update stats to reflect cleared search
            try {
              await fetchGlobalStats({ searchOverride: "" });
            } catch (err) {
              console.error("Error updating stats after clearing search:", err);
            }
          } catch (error) {
    ...
    showConfirm(
      ...
      async () => {
        try {
          ...
          showError(
            err.response?.data?.message || "Error deactivating product"
          );
  ...
  return (
    <div className="product-manager-main-container">
      <h1 className="product-manager-title">
        <IoBagHandleSharp color="#363636b7" /> Manage Products
      </h1>
      <p className="product-manager-subtitle">Manage your product catalog</p>

      {/* Search and Filters combined in one row - search moved into the controls below */}
      ...
      <div className="search-filter-controls">
        ...
        <div className="status-filter-container">
          ...
          >
            <option value="all">
              All Products ({globalStats.totalProducts})
            </option>
            <option value="active">
              Active ({globalStats.activeProducts})
            </option>
            <option value="inactive">
              Inactive ({globalStats.inactiveProducts})
            </option>
          </select>
        </div>
        ...
        <table className="products-table">
          <thead>
            <tr className="table-header-row">
              ...
              <th
                className={`table-header-cell sortable ${
                  sortConfig.field === "category_id" ? "active" : ""
                }`}
                onClick={() => handleSort("category_id")}
              >
                Category
                <span className="sort-arrow">
                  {sortConfig.field === "category_id" ? (
                    sortConfig.direction === "asc" ? (
                      <FaSortUp />
                    ) : (
                      <FaSortDown />
              ...
              <th
                className={`table-header-cell sortable ${
                  sortConfig.field === "is_active" ? "active" : ""
                }`}
          ...
          <tbody>
                ...
                <td className="table-body-cell">
                  <div className="price-display">
                    <div className="base-price">
                      Base: {formatBasePrice(p.price, currency)}
                    </div>
                    <div className="final-price">
                      Final: {formatPriceWithTax(p.price, currency, vat_rate)}
                    </div>
                  </div>
                </td>
                <td className="table-body-cell">
                  <div className="table-stock-info">
                    <div
                      className={`table-stock-indicator ${
                        getStockLevel(p.stock || 0).class
                      }`}
                    >
                      {p.stock || 0} - {getStockLevel(p.stock || 0).text}
                    </div>
                  </div>
                </td>
                  ...
                  <div className="table-actions">
                    ...
                    {p.is_active ? (
                      ...
                      <button
                        ...
                        title="Restore"
                      >
                        <MdOutlineAutorenew size={18} /> Restore
                      </button>
                    )}
                  </div>
                </td>
            ...
            {products.length === 0 && (
              <tr>
                <td colSpan="7" className="table-body-cell no-products-message">
                  No products found.
                </td>
              </tr>
            )}
