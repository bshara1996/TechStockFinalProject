...
const PromotionsBanner = () => {
  ...
  const formatApplicableItems = (promotion) => {
    ...
    // If no items found, promotion is invalid (must have specific products or categories)
    if (items.length === 0) {
  ...
  // Filter promotions that have valid applicable items (only after products/categories are loaded)
  const validPromotions = useMemo(() => {
    // Don't filter if products or categories haven't loaded yet
    if (products.length === 0 && categories.length === 0) {
      return promotions;
    }
    
    return promotions.filter(promotion => {
      // Only show promotions that have specific products or categories
      const items = getApplicableItems(promotion);
      return items.length > 0;
    });
  }, [promotions, products, categories]);

  if (!isVisible || validPromotions.length === 0) {
    return null;
  }
