    ...
    <div className="modal-overlay" onClick={onClose}>
      ...
      >
        ...
        <div className="promo-details-body">
          ...
          {/* Error message if no specific items (invalid promotion) - only show after loading completes */}
          {!loadingItems && applicableProducts.length === 0 && applicableCategories.length === 0 && (() => {
            // Check if promotion actually has applicable_products or applicable_categories defined in the data
            ...
            // Only show error if promotion doesn't have any applicable items defined in the data
            // (not just if the API calls returned empty - they might have failed or items don't exist)
            if (!hasApplicableProducts && !hasApplicableCategories) {
              return (
                <div className="promo-detail-row">
                  <span className="promo-detail-label">Applies To:</span>
                  <span className="promo-detail-value" style={{ color: '#d32f2f' }}>
                    Invalid promotion - must specify products or categories
                  </span>
                </div>
              );
            }
            // If promotion has IDs but nothing was fetched, show a different message
            return (
              <div className="promo-detail-row">
                <span className="promo-detail-label">Applies To:</span>
                <span className="promo-detail-value" style={{ color: '#ff9800' }}>
                  Unable to load applicable items. Please try again later.
                </span>
              </div>
            );
          })()}
